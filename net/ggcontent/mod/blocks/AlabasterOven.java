...
public class AlabasterOven extends BlockContainer{
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon(Ggcontent.MODID + ":" + "AlabasterOvenSide");
		this.iconfront = iconRegister.registerIcon(Ggcontent.MODID + ":" + (this.IsActive ? "AlabasterOvenFrontOn" : "AlabasterOvenFrontOff" ));
	    this.icontop = iconRegister.registerIcon(Ggcontent.MODID + ":" + "AlabasterOvenTop");
	}
	...
	public Item getItemDropped(int i, Random random, int j) {
		return Item.getItemFromBlock(Ggcontent.blockAlabasterOvenIdle);
	...
	private void setDefaultDirection(World world, int x, int y, int z) {
	 ...
	 public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
	 	 if(!world.isRemote) {
	 		 FMLNetworkHandler.openGui(player, Ggcontent.instance, Ggcontent.guiIDalabasterOven, world, x, y, z);
	 	 }
	 	 return true;
	 }
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random) {
		if(this.IsActive) {
			...
			if(direction == 4){
				world.spawnParticle("largesmoke", (double)(x1 - f), (double)(y1), (double)(z1 + f1), 0D, 0D, 0D);
			...
			if(direction == 2){
				world.spawnParticle("largesmoke", (double)(x1 + f1), (double)(y1), (double)(z1 - f), 0D, 0D, 0D);
	...
	public static void updateAlabasterOvenBlockState(boolean active, World worldObj,	int xCoord, int yCoord, int zCoord) {
		...
		if(active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, Ggcontent.blockAlabasterOvenActive);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, Ggcontent.blockAlabasterOvenIdle);
		}
